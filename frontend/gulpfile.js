...
gulp.task('copy', function() {
  ...
  var blockly = gulp.src(['app/blockly/**/*.js', ], {dot: true})
  ...
  return merge(app, blockly, bower, elements, vulcanized, swBootstrap,
               swToolbox).pipe($.size({title: 'copy'}));
});
...
gulp.task('html', function() {
  return optimizeHtmlTask(['app/**/*.html', '!app/{elements,test}/**/*.html'],
                          dist());
});
...
// This include a list of files that should be precached, as well as a
// (hopefully unique) cache
// id that ensure that multiple PSK projects don't share the same Cache Storage.
// This task does not run by default, but if you are interested in using service
// worker caching
// in your project, please enable it within the 'default' task.
// See
// https://github.com/PolymerElements/polymer-starter-kit#enable-service-worker-support
// for more context.
...
gulp.task('default', ['clean'], function(cb) {
  ...
  runSequence(['copy', 'styles'], 'elements',
...
gulp.task('deploy-gh-pages', function() {
  return gulp.src(dist('**/*'))
      ...
      .pipe(
          $.if (process.env.TRAVIS === 'true', $.ghPages({
                  remoteUrl:
